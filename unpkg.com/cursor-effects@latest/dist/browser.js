  ...
  return (
    ...
    (t.clockCursor = function (t) {
      ...
      const y = (
        ...
        X = [],
        D = [];
      var T = parseInt(y.length + w + x.length + E.length + M.length) + 1;
      ...
      function z() {
        ...
        for (let t = 0; t < T; t++)
          (b[t] = 0), (R[t] = 0), (S[t] = 0), (Y[t] = 0);
        for (let t = 0; t < y.length; t++) D[t] = { color: a, value: y[t] };
        for (let t = 0; t < v.length; t++) X[t] = { color: r, value: v[t] };
        for (let t = 0; t < x.length; t++) I[t] = { color: A, value: x[t] };
        for (let t = 0; t < E.length; t++) H[t] = { color: u, value: E[t] };
        for (let t = 0; t < M.length; t++) W[t] = { color: d, value: M[t] };
      ...
      function Q() {
        !(function () {
          (S[0] = Math.round((b[0] += (l.y - b[0]) * m))),
            (Y[0] = Math.round((R[0] += (l.x - R[0]) * m)));
          for (let t = 1; t < T; t++)
            (S[t] = Math.round((b[t] += (S[t - 1] - b[t]) * m))),
              (Y[t] = Math.round((R[t] += (Y[t - 1] - R[t]) * m))),
              b[t - 1] >= c - 80 && (b[t - 1] = c - 80),
              R[t - 1] >= h - 80 && (R[t - 1] = h - 80);
        })(),
          (function () {
            n.clearRect(0, 0, h, c);
            const t = new Date(),
            ...
            for (let t = 0; t < D.length; t++)
              (D[t].y = b[t] + 67.5 * Math.sin(-i + (t * L * Math.PI) / 180)),
                (D[t].x = R[t] + 67.5 * Math.cos(-i + (t * L * Math.PI) / 180)),
                (n.fillStyle = D[t].color),
                n.fillText(D[t].value, D[t].x, D[t].y);
            for (let t = 0; t < X.length; t++)
              (X[t].y =
                b[D.length + t] + 45 * Math.sin((t * C * Math.PI) / 180)),
                (X[t].x =
                  R[D.length + t] + 45 * Math.cos((t * C * Math.PI) / 180)),
                (n.fillStyle = X[t].color),
                n.fillText(X[t].value, X[t].x, X[t].y);
            for (let t = 0; t < I.length; t++)
              (I[t].y = b[D.length + w + t] + 0 + t * B * Math.sin(a)),
                (I[t].x = R[D.length + w + t] + 0 + t * B * Math.cos(a)),
                (n.fillStyle = I[t].color),
                n.fillText(I[t].value, I[t].x, I[t].y);
            for (let t = 0; t < H.length; t++)
              (H[t].y =
                b[D.length + w + I.length + t] + 0 + t * B * Math.sin(s)),
                (H[t].x =
                  R[D.length + w + I.length + t] + 0 + t * B * Math.cos(s)),
                (n.fillStyle = H[t].color),
                n.fillText(H[t].value, H[t].x, H[t].y);
            for (let t = 0; t < W.length; t++)
              (W[t].y =
                b[D.length + w + I.length + H.length + t] +
                0 +
                t * B * Math.sin(i)),
                (W[t].x =
                  R[D.length + w + I.length + H.length + t] +
                  0 +
                  t * B * Math.cos(i)),
                (n.fillStyle = W[t].color),
                n.fillText(W[t].value, W[t].x, W[t].y);
          })(),
          (i = requestAnimationFrame(Q));
      }
      function Z() {
        e.remove(),
          cancelAnimationFrame(i),
