...
function r(t) {
  ...
  function z() {
    ...
    (e = document.createElement("canvas")),
      ...
      (n.textAlign = "center"),
      (n.textBaseline = "middle");
    for (let t = 0; t < D; t++) (R[t] = 0), (S[t] = 0), (b[t] = 0), (Y[t] = 0);
    for (let t = 0; t < y.length; t++) T[t] = { color: a, value: y[t] };
  ...
  function Q() {
    !(function () {
      (b[0] = Math.round((R[0] += (l.y - R[0]) * m))),
        (Y[0] = Math.round((S[0] += (l.x - S[0]) * m)));
      for (let t = 1; t < D; t++)
        (b[t] = Math.round((R[t] += (b[t - 1] - R[t]) * m))),
          (Y[t] = Math.round((S[t] += (Y[t - 1] - S[t]) * m))),
          R[t - 1] >= c - 80 && (R[t - 1] = c - 80),
          S[t - 1] >= h - 80 && (S[t - 1] = h - 80);
    })(),
      (function () {
        n.clearRect(0, 0, h, c);
        const t = new Date(),
        ...
        for (let t = 0; t < T.length; t++)
          (T[t].y = R[t] + 67.5 * Math.sin(-i + (t * C * Math.PI) / 180)),
            (T[t].x = S[t] + 67.5 * Math.cos(-i + (t * C * Math.PI) / 180)),
            (n.fillStyle = T[t].color),
            n.fillText(T[t].value, T[t].x, T[t].y);
        for (let t = 0; t < X.length; t++)
          (X[t].y = R[T.length + t] + 45 * Math.sin((t * L * Math.PI) / 180)),
            (X[t].x = S[T.length + t] + 45 * Math.cos((t * L * Math.PI) / 180)),
            (n.fillStyle = X[t].color),
            n.fillText(X[t].value, X[t].x, X[t].y);
        for (let t = 0; t < I.length; t++)
          (I[t].y = R[T.length + w + t] + 0 + t * B * Math.sin(a)),
            (I[t].x = S[T.length + w + t] + 0 + t * B * Math.cos(a)),
            (n.fillStyle = I[t].color),
            n.fillText(I[t].value, I[t].x, I[t].y);
        for (let t = 0; t < H.length; t++)
          (H[t].y = R[T.length + w + I.length + t] + 0 + t * B * Math.sin(s)),
            (H[t].x = S[T.length + w + I.length + t] + 0 + t * B * Math.cos(s)),
            (n.fillStyle = H[t].color),
            n.fillText(H[t].value, H[t].x, H[t].y);
        for (let t = 0; t < W.length; t++)
          (W[t].y =
            R[T.length + w + I.length + H.length + t] +
            0 +
            t * B * Math.sin(i)),
            (W[t].x =
              S[T.length + w + I.length + H.length + t] +
              0 +
              t * B * Math.cos(i)),
